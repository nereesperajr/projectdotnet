var IndexJS = function () {
    ...
    function clientList(list) {
        ...
        $.each(list, function (idx, val) {
            ...
            _html += initHTML.ITEM({ H2: val.clientName, P: val.address, ID: "" });               
            cnt++;
        });
    ...
    function buttonEvents() {
        ...
        $("#searchdeceasedbtn").off().on('click', function () {
            ...
            $.ajax({
                type: 'POST',
                url: 'WebService/Index.asmx/GetDeceasedInfo',
                dataType: 'json',
                data: JSON.stringify({
                    name: _selectedDeceased
                }),
                contentType: 'application/json; charset=utf-8',
                success: function (response) {
                    _result = response.d.Deceased;
                },
                error: function (error) {
                    CommonJS.notify({ title: "", msg: _result.msg }, "bottom", "right");
                },
